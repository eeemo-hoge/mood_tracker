...
double calculateStdDev(const vector<double>& data) {
    return sqrt(calculateVariance(data));
}

double calculatePValue(const vector<double>& data1, const vector<double>& data2) {
    ...
    double var2 = calculateVariance(data2);
    double n1 = data1.size();
    double n2 = data2.size();
    double t = (mean1 - mean2) / sqrt(var1/n1 + var2/n2);
    return 2.0 * (1.0 - 0.5 * (1.0 + erf(t / sqrt(2.0))));
}

void SignificantData() {
    ifstream fin("analysis.csv");
    if(!fin){
        cout << "Error opening file" << endl;
        return;
    }
    ...
    string line;
    vector<vector<double>> columns(14);
    int lineNum = 0;
    
    cout << "Significant Data (Average values for each period):" << endl;
    cout << "Period, Anger, Hunger, Thirstty, Numbness, LeadingToExist, Sanity, ReadSpeed, BrainSpeed, DopamineLevel, Sleepiness, PainOfHand, Earwarm, BPM" << endl;
    ...
    while(getline(fin, line)) {
        ...
        if(lineNum > 0) {
            ...
            string token;
            int colIdx = 0;
            while(getline(ss, token, ',') && colIdx < 14) {
                if(colIdx > 0) {
                    columns[colIdx].push_back(stod(token));
                }
                colIdx++;
            }
        }
    ...
    if(lineNum > 1) {
        ...
        vector<string> labels = {"Anger", "Hunger", "Thirstty", "Numbness", "LeadingToExist", "Sanity", "ReadSpeed", "BrainSpeed", "DopamineLevel", "Sleepiness", "PainOfHand", "Earwarm", "BPM"};
        for(int i = 1; i < 14; i++) {
            cout << labels[i-1] << ":" << endl;
            cout << "  Mean: " << fixed << setprecision(2) << calculateMean(columns[i]) << endl;
            cout << "  Median: " << calculateMedian(columns[i]) << endl;
            cout << "  Variance: " << calculateVariance(columns[i]) << endl;
            cout << "  Std Dev: " << calculateStdDev(columns[i]) << endl;
        }
    }
}
...
int main(){
    ...
    cout << "Log system started\n 1=View CSV\n 2=Input data\n 3=Exit\n 4=Analyze CSV\n 5=Show significant data" << endl;
    cout << "-------------------------------------------------------------" << endl;
    ...
    while(true) {
        ...
        if(cmd == 1) {
            ...
            analyzeCSV();
            system("cat analysis.csv");
        }
        else if(cmd == 5) {
            cout << "Showing significant data..." << endl;
            SignificantData();
